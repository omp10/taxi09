...
const isAndroidWebView = () => {
  if (globalThis.window?.__isTaxi09WebView) return true;
...
const callNativeInterface = (targetUrl, checkoutPayload) => {
  if (typeof globalThis.Taxi09Native?.openExternalUrl === 'function') {
    try {
      globalThis.Taxi09Native.openExternalUrl({ url: targetUrl });
      recordCheckoutDiagnostic({ status: 'Taxi09-native-bridge-called', targetUrl });
      return true;
    } catch (error) {
      recordCheckoutDiagnostic({ status: 'Taxi09-native-bridge-failed', message: error?.message || String(error) });
    }
  }
...
const redirectInCurrentWindow = (targetUrl, status = 'browser-redirect') => {
  ...
  if (isAndroidWebView() || globalThis.window?.__isTaxi09WebView) {
