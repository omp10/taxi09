...
const compactLabelClass = 'mb-2 block text-[11px] font-bold uppercase tracking-[0.16em] text-slate-400';
const isTwoWheelerCategory = (value = '') => String(value || '').trim().toLowerCase() === 'bike';
...
const DEFAULT_SUBSCRIPTION_PLANS = [
  { id: 'sub-day-1', label: '1 Day', durationDays: 1, price: '', includedKm: 120, extraKmPrice: '', deposit: '', active: true },
  { id: 'sub-day-7', label: '7 Days', durationDays: 7, price: '', includedKm: 700, extraKmPrice: '', deposit: '', active: true },
  { id: 'sub-day-30', label: '1 Month', durationDays: 30, price: '', includedKm: 3000, extraKmPrice: '', deposit: '', active: true },
];
...
const normalizeAddOns = (items = []) =>
  (Array.isArray(items) ? items : []).map((item, index) => ({
    id: String(item.id || `addon-${index + 1}`),
    label: String(item.label || ''),
    description: String(item.description || ''),
    price: normalizeNumberInput(item.price),
    originalPrice: normalizeNumberInput(item.originalPrice),
    icon: String(item.icon || 'Package'),
    active: item.active !== false,
  }));

const normalizeSubscriptionPlans = (items = DEFAULT_SUBSCRIPTION_PLANS) =>
  (Array.isArray(items) && items.length ? items : DEFAULT_SUBSCRIPTION_PLANS).map((item, index) => ({
    id: String(item.id || `sub-plan-${index + 1}`),
    label: String(item.label || `${item.durationDays || index + 1} Day Plan`),
    durationDays: normalizeNumberInput(item.durationDays),
    price: normalizeNumberInput(item.price),
    includedKm: normalizeNumberInput(item.includedKm),
    extraKmPrice: normalizeNumberInput(item.extraKmPrice),
    deposit: normalizeNumberInput(item.deposit),
    active: item.active !== false,
  }));
...
const buildDefaultForm = () => {
  return {
    ...
    subscription: {
      enabled: false,
      plans: normalizeSubscriptionPlans(),
    },
    blueprint: null,
...
const RentalVehicleTypes = ({ mode: propMode }) => {
  ...
  const [rentalSubcategories, setRentalSubcategories] = useState([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setRentalSubcategories(Array.isArray(subcategoryResults) ? subcategoryResults : []);
        ...
        if (id) {
          ...
          if (selected) {
            setFormData({
              ...
              rentalSubcategoryId: selected.rentalSubcategoryId || '',
              rentalSubcategoryName: selected.rentalSubcategoryName || '',
              ...
              subscription: {
                enabled: Boolean(selected.subscription?.enabled),
                plans: normalizeSubscriptionPlans(selected.subscription?.plans),
              },
  ...
  const availableTwoWheelerSubcategories = useMemo(
    () =>
      rentalSubcategories.filter(
        (item) => item.active !== false && isTwoWheelerCategory(item.vehicleCategory || 'Bike'),
      ),
    [rentalSubcategories],
  );
  ...
  const addAddOn = () => {
    setFormData((current) => ({
      ...
      addOns: [
        ...(current.addOns || []),
        { id: `addon-${(current.addOns?.length || 0) + 1}`, label: '', description: '', price: 0, originalPrice: 0, icon: 'Package', active: true },
      ],
    }));
  };
  ...
  const updateSubscriptionPlan = (planId, field, value) => {
    setFormData((current) => ({
      ...current,
      subscription: {
        ...current.subscription,
        plans: (Array.isArray(current.subscription?.plans) ? current.subscription.plans : []).map((item) =>
          item.id === planId
            ? {
                ...item,
                [field]: ['durationDays', 'price', 'includedKm', 'extraKmPrice', 'deposit'].includes(field)
  ...
  const addSubscriptionPlan = () => {
    setFormData((current) => ({
      ...
      subscription: {
        ...current.subscription,
        plans: [
          ...(Array.isArray(current.subscription?.plans) ? current.subscription.plans : []),
          {
  ...
  const removeSubscriptionPlan = (planId) => {
    setFormData((current) => ({
      ...current,
      subscription: {
        ...current.subscription,
        plans: (Array.isArray(current.subscription?.plans) ? current.subscription.plans : []).filter((item) => item.id !== planId),
      },
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...formData,
        rentalSubcategoryId: isTwoWheelerCategory(formData.vehicleCategory) ? formData.rentalSubcategoryId : '',
        rentalSubcategoryName: isTwoWheelerCategory(formData.vehicleCategory) ? formData.rentalSubcategoryName : '',
        ...
        subscription: {
          enabled: Boolean(formData.subscription?.enabled),
          plans: (Array.isArray(formData.subscription?.plans) ? formData.subscription.plans : []).map((item) => ({
            ...item,
            durationDays: Math.max(1, toNumberOrZero(item.durationDays)),
            price: toNumberOrZero(item.price),
            includedKm: toNumberOrZero(item.includedKm),
            extraKmPrice: toNumberOrZero(item.extraKmPrice),
            deposit: toNumberOrZero(item.deposit),
          })),
        },
      };
      ...
      if (payload.subscription?.enabled) {
        const activePlans = payload.subscription.plans.filter((item) => item.active !== false);
        if (!activePlans.length) {
          throw new Error('Please keep at least one active subscription plan');
        }
      }
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-slate-500">
                      {item.vehicleCategory || 'Rental vehicle'}
                      {item.rentalSubcategoryName ? ` • ${item.rentalSubcategoryName}` : ''}
                    </p>
                  </div>
                </div>
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl bg-slate-50 px-4 py-3 col-span-2">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Subscription</p>
                  <p className="mt-1 text-sm font-bold text-slate-900">
                    {selectedItem.subscription?.enabled ? `${selectedItem.subscription?.plans?.filter((item) => item.active !== false).length || 0} plans enabled` : 'Disabled'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Sub Category</p>
                    <p className="mt-1 text-sm font-semibold text-slate-900">{selectedItem.rentalSubcategoryName || 'Not assigned'}</p>
                  </div>
                  <div>
              ...
              <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                {selectedItem.subscription?.enabled ? (
                  <div className="mt-4 space-y-3">
                    {(selectedItem.subscription?.plans || []).filter((item) => item.active !== false).map((plan) => (
                      <div key={plan.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{plan.durationDays} days</p>
                          </div>
                          <p className="text-base font-black text-slate-900">Rs {plan.price}</p>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600">
                          <span className="rounded-full bg-white px-3 py-1">Included km: {plan.includedKm}</span>
                          <span className="rounded-full bg-white px-3 py-1">Extra km: Rs {plan.extraKmPrice}</span>
                          <span className="rounded-full bg-white px-3 py-1">Deposit: Rs {plan.deposit}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="mt-4 text-sm text-slate-500">Subscription plans are disabled for this vehicle.</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
        <div className="grid grid-cols-1 gap-8 p-6 lg:grid-cols-2 lg:p-8">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                setFormData((current) => ({
                  ...
                  rentalSubcategoryId: isTwoWheelerCategory(nextCategory) ? current.rentalSubcategoryId : '',
                  rentalSubcategoryName: isTwoWheelerCategory(nextCategory) ? current.rentalSubcategoryName : '',
                }));
              }}
              className={inputClass}
            >
          ...
          <div>
            ...
            <select
              value={formData.rentalSubcategoryId}
              onChange={(event) => {
                const selectedSubcategory = availableTwoWheelerSubcategories.find(
                  (item) => String(item.id || item._id) === String(event.target.value),
                );
                setFormData((current) => ({
                  ...current,
                  rentalSubcategoryId: event.target.value,
                  rentalSubcategoryName: selectedSubcategory?.name || '',
                }));
              }}
              className={inputClass}
              disabled={!isTwoWheelerCategory(formData.vehicleCategory)}
            >
              <option value="">
                {isTwoWheelerCategory(formData.vehicleCategory)
                  ? 'Select sub category'
                  : 'Available only for two wheeler'}
              </option>
              {availableTwoWheelerSubcategories.map((option) => (
            ...
            <p className="mt-2 text-xs text-slate-500">
              Use this for bike or two wheeler rentals like scooty, sports bike, or EV bike.
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-[24px] border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-slate-50 p-4">
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-sm font-semibold text-slate-500">
                    {formData.short_description || 'Your hourly rental setup preview appears here.'}
                  </p>
                  <p className="truncate text-xs font-semibold uppercase tracking-wide text-slate-400">
                    {formData.vehicleCategory || 'Vehicle'}
                    {formData.rentalSubcategoryName ? ` • ${formData.rentalSubcategoryName}` : ''}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl bg-white px-4 py-3 shadow-sm col-span-2">
                  <div className="flex items-center gap-2 text-slate-500">
                    <Clock3 size={15} />
                    <span className="text-[11px] font-bold uppercase tracking-wide">Subscriptions</span>
                  </div>
                  <p className="mt-1 text-lg font-black text-slate-900">
                    {formData.subscription?.enabled
                      ? `${(formData.subscription?.plans || []).filter((item) => item.active !== false).length} plans`
                      : 'Disabled'}
                  </p>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <label className="text-sm font-bold text-slate-900">Add-ons</label>
                  <p className="mt-1 text-xs text-slate-500">
                    Optional extras offered with this vehicle. Prices are charged per booking and applied server-side.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={addAddOn}
                  className="shrink-0 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                >
                  + Add
                </button>
              </div>
                    ...
                    <div key={index} className="rounded-2xl border border-slate-100 bg-slate-50/80 p-3">
                      <div className="grid gap-3 sm:grid-cols-[1fr_120px_120px_auto]">
                        <div>
                          ...
                          <input
                            value={addOn.label}
                            onChange={(event) => updateAddOn(index, 'label', event.target.value)}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            value={addOn.price}
                            onChange={(event) => updateAddOn(index, 'price', Number(event.target.value || 0))}
                            className={inputClass}
                          />
                        </div>
                        <div>
                          <label className={labelClass}>Was (₹)</label>
                          <input
                            type="number"
                            min="0"
                            value={addOn.originalPrice}
                            onChange={(event) => updateAddOn(index, 'originalPrice', Number(event.target.value || 0))}
                            className={inputClass}
                            placeholder="0"
                          />
                          {addOn.originalPrice > 0 && addOn.originalPrice <= addOn.price && (
                            <p className="mt-1 text-[11px] font-semibold text-amber-600">
                              Must exceed price to show as a discount
                            </p>
                          )}
                        </div>
                        <div className="flex items-end gap-2">
                          <button
                            type="button"
                            onClick={() => updateAddOn(index, 'active', !addOn.active)}
                            className={`h-10 rounded-xl px-3 text-xs font-bold ${addOn.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}
                          >
                          ...
                          <button
                            ...
                            className="h-10 rounded-xl bg-rose-50 px-3 text-xs font-bold text-rose-600 hover:bg-rose-100"
                          >
                            Remove
                          </button>
                        </div>
                      ...
                      <div className="mt-3">
                        ...
                        <input
                          ...
                          placeholder="Safe & comfortable seating for kids"
                          className={inputClass}
                        />
                      </div>
                    </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <label className="text-sm font-bold text-slate-900">Enable Subscription</label>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
                    Turn this on to add Revv-style rental subscription plans like 1 day, 7 days, and 1 month for this vehicle.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => updateSubscription('enabled', !formData.subscription?.enabled)}
                  className={`relative h-7 w-14 rounded-full transition-all ${formData.subscription?.enabled ? 'bg-violet-500' : 'bg-slate-300'}`}
                  aria-pressed={formData.subscription?.enabled}
                >
                  <span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition-all ${formData.subscription?.enabled ? 'left-8' : 'left-1'}`} />
                </button>
              </div>
              ...
              {formData.subscription?.enabled ? (
                <div className="mt-4 space-y-4 rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <h3 className="text-sm font-bold text-slate-900">Subscription Pricing Plans</h3>
                      <p className="mt-1 text-xs text-slate-500">Set total price, included km, extra km charge, and deposit for each plan.</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-4">
                    {(formData.subscription?.plans || []).map((plan) => (
                      <div key={plan.id} className="rounded-[22px] border border-slate-200 bg-white p-4">
                        <div className="mb-4 flex items-center justify-between gap-3">
                          <div>
                            <p className="text-sm font-black text-slate-900">{plan.label || 'Subscription Plan'}</p>
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Vehicle subscription</p>
                          </div>
                          {(formData.subscription?.plans || []).length > 1 ? (
                            <button
                              type="button"
                        ...
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                          <div>
                            <label className={compactLabelClass}>Plan Label</label>
                            <input
                              value={plan.label}
                          ...
                          <div>
                            <label className={compactLabelClass}>Days</label>
                            <input
                              type="number"
                              value={plan.durationDays}
                              onChange={(event) => updateSubscriptionPlan(plan.id, 'durationDays', event.target.value)}
                              className={inputClass}
                              placeholder="30"
                          ...
                          <div>
                            <label className={compactLabelClass}>Total Price</label>
                            <input
                              type="number"
                              value={plan.price}
                          ...
                          <div>
                            <label className={compactLabelClass}>Included Km</label>
                            <input
                              type="number"
                              value={plan.includedKm}
                              onChange={(event) => updateSubscriptionPlan(plan.id, 'includedKm', event.target.value)}
                              className={inputClass}
                              placeholder="3000"
                          ...
                          <div>
                            <label className={compactLabelClass}>Extra Km Charge</label>
                            <input
                              type="number"
                              value={plan.extraKmPrice}
                              onChange={(event) => updateSubscriptionPlan(plan.id, 'extraKmPrice', event.target.value)}
                              className={inputClass}
                              placeholder="8"
                          ...
                          <div>
                            <label className={compactLabelClass}>Refundable Deposit</label>
                            <input
                              type="number"
                              value={plan.deposit}
                              onChange={(event) => updateSubscriptionPlan(plan.id, 'deposit', event.target.value)}
                              className={inputClass}
                              placeholder="5000"
                        ...
                        <label className="mt-4 flex items-center gap-2 text-xs font-semibold text-slate-600">
                          <input
                            ...
                            onChange={(event) => updateSubscriptionPlan(plan.id, 'active', event.target.checked)}
                            className="h-4 w-4 rounded border-slate-300"
                          />
                          Keep this subscription plan active
                        </label>
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              {formData.pricing.map((price) => (
                <div key={price.id} className="rounded-[26px] border border-slate-200 bg-slate-50/70 p-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    <div>
                      <label className={compactLabelClass}>Package Label</label>
                      <input
                        value={price.label}
                    ...
                    <div>
                      <label className={compactLabelClass}>Hours</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className={compactLabelClass}>Base Rent</label>
                      <input
                        type="number"
                        value={price.price}
                    ...
                    <div>
                      <label className={compactLabelClass}>Extra Hour Charge</label>
                      <input
                        type="number"
