...
const AppHeader = ({ showBack = false, subtitle = '' }) => {
  ...
  return (
    <header className="bg-[#fffdf8] h-[92px] flex items-center justify-between px-6 w-full sticky top-0 z-50 select-none">
      <button
      ...
      >
        {showBack ? <ArrowLeft size={24} strokeWidth={2.6} /> : <Menu size={26} strokeWidth={2.5} />}
      </button>
      ...
      >
        <svg viewBox="0 0 120 22" className="w-[104px] h-[16px] -mb-1" aria-hidden="true">
          <path
        ...
        <span className="text-[30px] font-black italic tracking-[-0.05em] text-slate-950 leading-none">
          Taxi<span className="text-[#F5B700]">09</span>
        </span>
        <span className="text-[8px] font-semibold tracking-[0.42em] text-slate-700 mt-1">
      ...
      >
        ...
        <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-[#F5B700] rounded-full border-2 border-white" />
      </button>
    </header>
  );
};
