  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#F8FAFC_0%,#F3F4F6_38%,#EEF2F7_100%)] max-w-lg mx-auto font-sans pb-32 relative overflow-hidden">
      ...
      <header className="bg-white/90 backdrop-blur-md px-5 pt-6 pb-4 sticky top-0 z-20 border-b border-white/80 shadow-[0_4px_20px_rgba(15,23,42,0.05)]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <h1 className="text-[20px] font-black tracking-tight text-slate-900">Airport Cab</h1>
          </div>
          <span className="text-[11px] font-black px-2.5 py-1 rounded-full border bg-blue-50 text-blue-600 border-blue-100">Fixed Fare</span>
        </div>
      </header>
      ...
      <div className="mx-5 mt-4 rounded-[20px] overflow-hidden bg-gradient-to-br from-blue-600 to-blue-800 p-5 flex items-center gap-4 shadow-[0_8px_24px_rgba(37,99,235,0.25)]">
        <div className="flex-1">
          <p className="text-[12px] font-black text-blue-200 uppercase tracking-widest mb-1">On-time transfers</p>
          <h2 className="text-[19px] font-black text-white leading-tight">Indore Airport<br />Cab Booking</h2>
          <p className="text-[13px] font-bold text-blue-200 mt-1">Fixed fares · No surge pricing</p>
        </div>
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        <div>
          ...
          {errors.terminal && <p className="text-[13px] font-black text-red-500 ml-1 mt-1 flex items-center gap-1"><AlertCircle size={11} strokeWidth={3} />{errors.terminal}</p>}
        </div>

        {/* Date + Time */}
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[12px] font-black uppercase tracking-[0.22em] text-slate-400 ml-1 mb-1.5 block flex items-center gap-1"><Calendar size={10} strokeWidth={3} /> Date</label>
            <input type="date" value={date} min={new Date().toISOString().split('T')[0]}
              onChange={e => { setDate(e.target.value); setErrors(p => ({ ...p, date: '' })); }}
              className={`w-full rounded-[14px] px-3 py-3 text-[14.5px] font-bold text-slate-900 border-2 focus:outline-none transition-all ${errors.date ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-white/90'}`} />
            {errors.date && <p className="text-[12px] font-black text-red-500 ml-1 mt-1">{errors.date}</p>}
          </div>
          <div>
            <label className="text-[12px] font-black uppercase tracking-[0.22em] text-slate-400 ml-1 mb-1.5 block flex items-center gap-1"><Clock size={10} strokeWidth={3} /> Time</label>
            <input type="time" value={time}
              onChange={e => { setTime(e.target.value); setErrors(p => ({ ...p, time: '' })); }}
              className={`w-full rounded-[14px] px-3 py-3 text-[14.5px] font-bold text-slate-900 border-2 focus:outline-none transition-all ${errors.time ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-white/90'}`} />
            {errors.time && <p className="text-[12px] font-black text-red-500 ml-1 mt-1">{errors.time}</p>}
          </div>
        </div>

        {/* Vehicle selection */}
        <div>
          <p className="text-[12px] font-black uppercase tracking-[0.26em] text-slate-400 mb-2">Choose Vehicle</p>
              ...
              <motion.button key={v.id} whileTap={{ scale: 0.98 }} onClick={() => setVehicle(v.id)}
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[15.5px] font-black text-slate-900">{v.name}</span>
                    <span className="text-[11px] font-black px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100">Fixed Fare</span>
                  </div>
                  <p className="text-[13px] font-bold text-slate-400">{v.desc} · {v.seats} seats</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[18px] font-black text-slate-900">₹{v.fare}</p>
                  <p className="text-[11px] font-bold text-slate-400">one way</p>
                </div>
              </motion.button>
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg px-5 pb-6 pt-3 bg-gradient-to-t from-[#EEF2F7] via-[#F3F4F6]/95 to-transparent pointer-events-none z-30">
        <div className="pointer-events-auto bg-white/90 rounded-[20px] border border-white/80 shadow-[0_4px_14px_rgba(15,23,42,0.06)] px-4 py-3 flex items-center justify-between mb-3">
          <div>
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Fixed Fare</p>
            <p className="text-[20px] font-black text-slate-900">₹{selectedVehicle?.fare}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Vehicle</p>
            <p className="text-[14.5px] font-black text-slate-700">{selectedVehicle?.name}</p>
          </div>
        </div>
        <motion.button whileTap={{ scale: 0.97 }} onClick={handleBook}
          className="pointer-events-auto w-full bg-blue-600 py-4 rounded-[18px] text-[16.5px] font-black text-white shadow-[0_8px_24px_rgba(37,99,235,0.25)] flex items-center justify-center gap-2">
