    ...
    return (
      <div className="min-h-screen bg-[linear-gradient(180deg,#F8FAFC_0%,#F3F4F6_38%,#EEF2F7_100%)] max-w-lg mx-auto font-sans flex flex-col items-center justify-center px-5 gap-5">
        ...
        <div className="text-center">
          <p className="text-[11px] font-black uppercase tracking-[0.26em] text-slate-400">Booking Confirmed</p>
          <h1 className="text-[22px] font-black text-slate-900 tracking-tight mt-0.5">You're booked!</h1>
          <p className="text-[13.5px] font-bold text-slate-400 mt-1">ID: <span className="text-slate-700 font-black">{bookingId}</span></p>
        </div>
        <div className="w-full rounded-[20px] border border-white/80 bg-white/90 shadow-[0_4px_14px_rgba(15,23,42,0.06)] px-5 py-4 space-y-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13.5px] font-black text-slate-800 truncate">{route.from}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-orange-500 shrink-0" />
            <span className="text-[13.5px] font-black text-slate-800 truncate">{route.to}</span>
          </div>
          <div className="border-t border-slate-50 pt-2.5 flex justify-between items-center">
            <div>
              <p className="text-[12px] font-bold text-slate-400">{date} · {route.departure}</p>
              <p className="text-[12px] font-bold text-slate-400">Seats: {seats?.map(s=>s.label).join(', ')}</p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#F8FAFC_0%,#F3F4F6_38%,#EEF2F7_100%)] max-w-lg mx-auto font-sans pb-28 relative overflow-hidden">
      ...
      <motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        className="bg-white/90 backdrop-blur-md px-5 pt-6 pb-4 sticky top-0 z-20 border-b border-white/80 shadow-[0_4px_20px_rgba(15,23,42,0.05)]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[11px] font-black uppercase tracking-[0.26em] text-slate-400">Confirm Booking</p>
            <h1 className="text-[19px] font-black tracking-tight text-slate-900 leading-tight">Review & Pay</h1>
          </div>
        </div>
      </motion.header>
      ...
      <div className="px-5 pt-4 space-y-3">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Trip Details</p>
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
              <span className="text-[14.5px] font-black text-slate-900 truncate">{route.from}</span>
            </div>
            <div className="ml-0.5 w-px h-3 border-l border-dashed border-slate-200" />
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-orange-500 shrink-0" />
              <span className="text-[14.5px] font-black text-slate-900 truncate">{route.to}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-[12px] font-bold text-slate-400 border-t border-slate-50 pt-2.5">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Fare Breakdown</p>
          ...
          <div className="border-t border-slate-50 pt-2 flex justify-between">
            <span className="text-[15.5px] font-black text-slate-900">Total</span>
            <span className="text-[19px] font-black text-slate-900">₹{total}</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Payment Method</p>
          <div className="grid grid-cols-3 gap-2">
            {PAYMENT_METHODS.map(({ id, label, sub, icon: Icon }) => (
              <motion.button key={id} whileTap={{ scale: 0.96 }} onClick={() => setMethod(id)}
                ...
                <span className={`text-[12px] font-black ${method === id ? 'text-slate-900' : 'text-slate-400'}`}>{label}</span>
              </motion.button>
            ))}
          </div>
        </motion.div>
