        ...
        <div>
          ...
          <div className="mt-4 rounded-[20px] bg-[var(--dh-surface)] p-6 ring-1 ring-[var(--dh-border)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="rounded-[9px] bg-[#FFF0B8] px-3 py-1 text-[14.5px] font-black text-slate-900">{category}</span>
                </div>
                {vehicle.short_description && (
                  <p className="mt-2 text-[15.5px] font-medium text-[var(--dh-muted)]">{vehicle.short_description}</p>
                )}

                <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2">
                  {vehicle.capacity > 0 && (
                    <span className="flex items-center gap-2 text-[15.5px] font-semibold text-[var(--dh-text)]">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              {vehicle.advancePayment?.enabled && (
                <span className="flex items-center gap-1.5 rounded-[9px] bg-[#FFFBEC] px-3 py-2 text-[14px] font-bold text-slate-800">
              ...
              <span className="flex items-center gap-1.5 rounded-[9px] bg-emerald-50 px-3 py-2 text-[14px] font-bold text-emerald-700">
                ...
                <div className="relative mt-6 flex h-[330px] items-center justify-center rounded-[16px] bg-[var(--dh-chip)]">
                  ...
                  {gallery.length > 1 && (
                    <>
                      ...
                      <span className="absolute bottom-3 rounded-full bg-[var(--dh-surface)] px-3 py-1 text-[13.5px] font-bold text-[var(--dh-text)] shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="rounded-[20px] bg-[var(--dh-surface)] p-6 ring-1 ring-[var(--dh-border)]">
              <h2 className="text-[20px] font-black tracking-[-0.02em] text-[var(--dh-text)]">Vehicle Specifications</h2>
              <dl className="mt-4 space-y-3">
                {specRows.map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between border-b border-[var(--dh-border)] pb-2.5 last:border-0">
                    <dt className="text-[15px] font-semibold text-[var(--dh-muted)]">{label}</dt>
                    <dd className="text-[15px] font-bold text-[var(--dh-text)]">{value}</dd>
                  </div>
                ))}
              </dl>
            </div>
            ...
            <div className="rounded-[20px] bg-[var(--dh-surface)] p-6 ring-1 ring-[var(--dh-border)]">
              <h2 className="text-[20px] font-black tracking-[-0.02em] text-[var(--dh-text)]">Features</h2>
              {amenities.length ? (
                <div className="mt-4 grid grid-cols-2 gap-3">
                  {amenities.map((amenity) => (
                    <span key={amenity} className="flex items-center gap-2.5 text-[15px] font-semibold text-[var(--dh-text)]">
              ...
              {vehicle.description && (
                <p className="mt-5 border-t border-[var(--dh-border)] pt-4 text-[15px] font-medium leading-[1.6] text-[var(--dh-muted)]">
                  {vehicle.description}
                </p>
              )}
            </div>
          ...
          <div className="mt-4 rounded-[20px] bg-[#FFFBEC] p-6">
            <p className="text-[15.5px] font-black text-slate-900">Why book with Taxi09?</p>
            <div className="mt-4 grid grid-cols-4 gap-4">
              {WHY_BOOK.map(({ icon: Icon, title, copy }) => (
                <div key={title} className="flex items-start gap-2.5">
                  <Icon size={24} className="mt-0.5 shrink-0 text-[#F5B700]" strokeWidth={2} />
                  <span>
                    <span className="block text-[14.5px] font-black text-slate-900">{title}</span>
                    <span className="mt-0.5 block text-[13.5px] font-semibold text-slate-600">{copy}</span>
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 grid grid-cols-4 gap-4 rounded-[18px] bg-[var(--dh-surface)] px-6 py-5 ring-1 ring-[var(--dh-border)]">
            {ASSURANCES.map(({ icon: Icon, title, copy }) => (
              <div key={title} className="flex items-center gap-3">
                ...
                <span>
                  <span className="block text-[14.5px] font-black text-[var(--dh-text)]">{title}</span>
        ...
        <aside className="sticky top-[100px] h-fit space-y-4">
          <div className="rounded-[20px] bg-[var(--dh-surface)] p-5 shadow-[0_10px_30px_rgba(15,23,42,0.08)] ring-1 ring-[var(--dh-border)]">
            <div className="flex items-center justify-between">
              <h2 className="text-[18px] font-black tracking-[-0.02em] text-[var(--dh-text)]">Select Your Plan</h2>
              <span className="text-[14px] font-bold text-[var(--dh-muted)]">Daily Price</span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[15.5px] font-black text-[var(--dh-text)]">
                        {plan.label || `${plan.durationHours || 24} Hours`}
                      </span>
                      <span className="block text-[13.5px] font-semibold text-[var(--dh-muted)]">
                        {plan.includedKm ? `${plan.includedKm} km included` : 'Unlimited KM'}
                        {plan.extraKmPrice ? ` · Extra ${formatMoney(plan.extraKmPrice)}/km` : ''}
                      </span>
                    </span>
                    <span className="shrink-0 text-right">
                      <span className="block text-[18px] font-black text-[var(--dh-text)]">{formatMoney(plan.price)}</span>
                    </span>
                  </button>
                );
              })}
              {plans.length === 0 && (
                <p className="rounded-[12px] bg-[var(--dh-chip)] px-4 py-5 text-center text-[14.5px] font-semibold text-[var(--dh-muted)]">
            ...
            <div className="mt-5 border-t border-[var(--dh-border)] pt-4">
              <h3 className="text-[16.5px] font-black text-[var(--dh-text)]">Price Summary</h3>

              {quote ? (
                <div className="mt-3 space-y-2.5">
                  <div className="flex justify-between text-[15px] font-semibold text-[var(--dh-muted)]">
                    <span>Base Fare</span>
                  ...
                  {quote.payableNow > 0 && (
                    <div className="flex justify-between text-[14.5px] font-bold text-[var(--dh-muted)]">
            ...
            {vehicle.advancePayment?.enabled && quote && (
              <div className="mt-4 flex items-start gap-2.5 rounded-[12px] bg-emerald-50 p-3.5">
                ...
                <span>
                  <span className="block text-[14.5px] font-black text-emerald-800">Book with a part payment</span>
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2.5 rounded-[13px] bg-[#F5B700] py-3.5 text-[17px] font-bold text-slate-950 shadow-[0_10px_24px_rgba(245,183,0,0.3)] transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          <div className="rounded-[18px] bg-[#FFFBEC] p-5">
            <p className="text-[15.5px] font-black text-slate-900">Need help choosing the right car?</p>
            <p className="mt-1 text-[14px] font-semibold text-slate-600">Our experts are here to assist you</p>
            <button
              onClick={() => navigate('/taxi/user/support')}
              className="mt-3.5 flex items-center gap-2 rounded-[11px] bg-[var(--dh-surface)] px-4 py-2.5 text-[15px] font-bold text-[var(--dh-text)] ring-1 ring-[var(--dh-border)]"
            >
              <Phone size={15} strokeWidth={2.6} /> Call Us Now
            </button>
          </div>

          <p className="flex items-start gap-2 px-1 text-[13.5px] font-medium text-[var(--dh-muted)]">
