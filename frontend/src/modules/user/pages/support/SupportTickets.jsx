  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#F8FAFC_0%,#F3F4F6_38%,#EEF2F7_100%)] max-w-lg mx-auto font-sans pb-12 relative overflow-hidden">
      ...
      <header className="bg-white/90 backdrop-blur-md px-5 pt-6 pb-4 sticky top-0 z-20 border-b border-white/80 shadow-[0_4px_20px_rgba(15,23,42,0.05)]">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <p className="text-[11px] font-black uppercase tracking-[0.26em] text-slate-400">Help Center</p>
            <h1 className="text-[20px] font-black tracking-tight text-slate-900">Support Tickets</h1>
          </div>
        ...
        <div className="flex gap-1.5 bg-slate-100/80 p-1 rounded-[14px]">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-1.5 rounded-[10px] text-[13px] font-black uppercase tracking-widest transition-all ${
      ...
      <div className="px-5 pt-4 space-y-2.5">
        {error ? (
          <div className="rounded-[14px] border border-red-100 bg-red-50 px-4 py-3 text-[13.5px] font-bold text-red-600">
            {error}
          </div>
        ) : null}
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
            ...
            <div>
              <p className="text-[16.5px] font-black text-slate-700">No tickets yet</p>
              <p className="text-[13.5px] font-bold text-slate-400 mt-1">Tap + to get help</p>
            </div>
          </div>
        )}
          ...
          <motion.button key={t.id} type="button"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <p className="text-[15.5px] font-black text-slate-900 leading-tight truncate flex-1">{t.title}</p>
                <span className={`text-[11px] font-black px-2 py-0.5 rounded-full border shrink-0 ${STATUS_STYLES[t.status] || STATUS_STYLES.pending}`}>
                  {t.status}
                </span>
              </div>
          ...
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-[19px] font-black text-slate-900">New Ticket</h3>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[13px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Ticket Title</label>
                  <select
                    value={titleId}
                    onChange={(event) => {
                    ...
                    className="w-full rounded-[14px] px-4 py-3 text-[15.5px] font-bold text-slate-900 border-2 border-slate-100 bg-slate-50 focus:outline-none"
                  >
                ...
                <div>
                  <label className="text-[13px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Or Custom Title</label>
                  <input type="text" value={customTitle} onChange={e => { setCustomTitle(e.target.value); setErrors(p => ({ ...p, title: '' })); }}
                    ...
                    className={`w-full rounded-[14px] px-4 py-3 text-[15.5px] font-bold text-slate-900 border-2 focus:outline-none transition-all placeholder:text-slate-300 ${errors.title ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-slate-50'}`} />
                  {errors.title && <p className="text-[13px] font-black text-red-500 ml-1 mt-1 flex items-center gap-1"><AlertCircle size={11} strokeWidth={3} />{errors.title}</p>}
                </div>
                <div>
                  <label className="text-[13px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Description</label>
                  <textarea value={description} onChange={e => { setDesc(e.target.value); setErrors(p => ({ ...p, description: '' })); }}
                    placeholder="Describe your issue in detail..."
                    rows={4}
                    className={`w-full rounded-[14px] px-4 py-3 text-[15.5px] font-bold text-slate-900 border-2 focus:outline-none transition-all placeholder:text-slate-300 resize-none ${errors.description ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-slate-50'}`} />
                  {errors.description && <p className="text-[13px] font-black text-red-500 ml-1 mt-1 flex items-center gap-1"><AlertCircle size={11} strokeWidth={3} />{errors.description}</p>}
                </div>
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={submitting}
                  className="w-full bg-slate-900 text-white py-4 rounded-[16px] text-[15.5px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-sm">
