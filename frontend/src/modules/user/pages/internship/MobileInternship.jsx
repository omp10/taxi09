  ...
  return (
    <div className="premium-theme mx-auto min-h-screen max-w-lg bg-[#fffdf8] pb-24">
      ...
      <div className="bg-gradient-to-b from-[#FFF9E6] to-[#fffdf8] px-4 pb-5 pt-1">
        <span className="inline-flex items-center gap-1.5 rounded-lg bg-[#FFF0B8] px-2 py-1 text-[11.5px] font-black uppercase tracking-wider text-[#9A6B00]">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-slate-900 px-3 py-3">
            {[
              ...
              <div key={label} className="text-center">
                <p className="text-[18px] font-black text-[#F5B700]">{value ?? 0}</p>
          ...
          <div className="mt-2 space-y-2">
            {mine.applications.map((row) => (
              <div key={row._id} className="rounded-xl border border-slate-100 bg-white px-3 py-2.5">
                <p className="text-[14px] font-bold text-slate-900">{row.trackTitle || row.courseTitle}</p>
            ...
            <div className="mt-2 space-y-2">
              {mine.certificates.map((certificate) => (
                <div key={certificate._id} className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2.5">
                  ...
                  <p className="font-mono text-[12.5px] text-emerald-700">{certificate.certificateNumber}</p>
                </div>
              ))}
            </div>
          ) : null}
      ...
      <div className="px-4 pt-4">
        ...
        {!loading && tab === 'tracks' ? (
          tracks.length === 0 ? (
            <p className="py-14 text-center text-[14.5px] text-slate-500">No tracks are open right now.</p>
          ) : (
                  ...
                  <div className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="text-[15.5px] font-black leading-snug text-slate-900">{track.title}</h3>
                        {track.durationLabel ? (
                          <p className="text-[13px] text-slate-500">{track.durationLabel}</p>
                        ) : null}
                      </div>
                    </div>

                    <p className="mt-2.5 text-[14px] leading-relaxed text-slate-600">{track.summary}</p>
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
                      ...
                      <button
                        ...
                        className="rounded-lg bg-[#F5B700] px-3.5 py-1.5 text-[13.5px] font-black text-slate-900 disabled:bg-slate-100 disabled:text-slate-500"
                      >
        ...
        {!loading && tab === 'courses' ? (
          courses.length === 0 ? (
            <p className="py-14 text-center text-[14.5px] text-slate-500">No courses right now.</p>
          ) : (
            <div className="space-y-3">
              {courses.map((course) => (
                <article key={course._id} className="rounded-2xl border border-slate-100 bg-white p-4">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-[15.5px] font-black leading-snug text-slate-900">{course.title}</h3>
                    <span className="shrink-0 rounded-md bg-slate-100 px-1.5 py-0.5 text-[12px] font-bold capitalize text-slate-600">
                      {course.mode}
                    </span>
                  </div>

                  <p className="mt-1.5 text-[14px] text-slate-600">{course.summary}</p>

                  <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[13px] text-slate-500">
                    {course.rating > 0 ? (
            ...
            {STEPS.map(([title, sub], index) => (
              <li key={title} className="flex gap-3 rounded-2xl border border-slate-100 bg-white p-3.5">
                ...
                <span>
                  ...
                  <span className="block text-[13.5px] text-slate-500">{sub}</span>
                </span>
              </li>
            ))}
          </ol>
      ...
      <div className="mt-5 px-4">
        ...
        >
          <span className="flex items-center gap-2.5">
            ...
            <span>
              <span className="block text-[14.5px] font-black text-slate-900">Verify a certificate</span>
              <span className="block text-[13px] text-slate-500">Check any Taxi09 certificate number</span>
            </span>
          </span>
