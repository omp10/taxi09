...
const isNativeCameraBridgeAvailable = () => {
  ...
  return Boolean(
    window?.Taxi09Native?.captureInspectionPhoto
...
const StepDocuments = () => {
  ...
  const handleCameraCapture = async (templateId, key, label) => {
    ...
    try {
      ...
      if (typeof window?.Taxi09Native?.captureInspectionPhoto === 'function') {
        normalizedResult = normalizeNativeCameraBridgeResult(
          await withBridgeTimeout(window.Taxi09Native.captureInspectionPhoto(payload)),
        );
      }
