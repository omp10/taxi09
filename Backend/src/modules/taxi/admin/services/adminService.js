...
const DEFAULT_RENTAL_SUBSCRIPTION_PLANS = [
  { id: 'sub-day-1', label: '1 Day', durationDays: 1, price: 0, includedKm: 120, extraKmPrice: 0, deposit: 0, active: true },
  { id: 'sub-day-7', label: '7 Days', durationDays: 7, price: 0, includedKm: 700, extraKmPrice: 0, deposit: 0, active: true },
  { id: 'sub-day-30', label: '1 Month', durationDays: 30, price: 0, includedKm: 3000, extraKmPrice: 0, deposit: 0, active: true },
];

const normalizeRentalSubscriptionPlan = (item = {}, index = 0) => ({
  id: sanitizeBusText(item.id, `sub-plan-${index + 1}`),
  label: sanitizeBusText(item.label, `${Math.max(1, sanitizeBusSeatPrice(item.durationDays, index + 1))} Day Plan`),
  durationDays: Math.max(1, sanitizeBusSeatPrice(item.durationDays, index + 1)),
  price: Math.max(0, sanitizeBusSeatPrice(item.price, 0)),
  includedKm: Math.max(0, sanitizeBusSeatPrice(item.includedKm, 0)),
  extraKmPrice: Math.max(0, sanitizeBusSeatPrice(item.extraKmPrice, 0)),
  deposit: Math.max(0, sanitizeBusSeatPrice(item.deposit, 0)),
  active: item.active === undefined ? true : normalizeBoolean(item.active),
});

const normalizeRentalSubscription = (value = {}, existing = {}) => {
  const enabled = normalizeBoolean(value?.enabled ?? existing?.enabled ?? false);
  const fallbackPlans =
    Array.isArray(existing?.plans) && existing.plans.length
      ? existing.plans.map((item, index) => normalizeRentalSubscriptionPlan(item, index))
      : DEFAULT_RENTAL_SUBSCRIPTION_PLANS.map((item, index) => normalizeRentalSubscriptionPlan(item, index));

  const plans = Array.isArray(value?.plans)
    ? value.plans.map((item, index) => normalizeRentalSubscriptionPlan(item, index))
    : fallbackPlans;
...
const normalizeRentalVehiclePayload = (payload = {}, existing = {}) => {
  ...
  return {
    ...
    rentalSubcategoryId:
      payload.rentalSubcategoryId !== undefined
        ? sanitizeBusText(payload.rentalSubcategoryId, '')
        : sanitizeBusText(existing.rentalSubcategoryId, ''),
    rentalSubcategoryName: sanitizeBusText(
      payload.rentalSubcategoryName,
      existing.rentalSubcategoryName || '',
    ),
    image: sanitizeBusText(payload.image, existing.image || ''),
    coverImage: sanitizeBusText(
      payload.coverImage,
    ...
    subscription: normalizeRentalSubscription(
...
const serializeRentalVehicleSubcategory = (item) => ({
  _id: item._id,
  id: item._id,
  name: item.name || '',
  short_description: item.short_description || '',
  description: item.description || '',
  vehicleCategory: item.vehicleCategory || 'Bike',
  status: item.status || (item.active === false ? 'inactive' : 'active'),
  active: item.active !== false,
  image: item.image || '',
  images: item.images || [],
...
export const forgotPassword = async (email) => {
  ...
  await sendEmail({
    to: email,
    subject: `Password Reset OTP for ${process.env.APP_NAME || 'Taxi09'}`,
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px;">
        ...
        <p style="font-size: 12px; color: #666;">Regards,<br>Team ${process.env.APP_NAME || 'Taxi09'}</p>
      </div>
    `,
  });
...
export const updateBusService = async (id, payload = {}, options = {}) => {
  ...
  export const createRentalVehicleType = async (payload = {}) => {
    ...
    if (normalizedPayload.subscription?.enabled) {
      const activePlans = (Array.isArray(normalizedPayload.subscription.plans) ? normalizedPayload.subscription.plans : []).filter(
        (item) => item.active !== false,
      );
      if (!activePlans.length) {
        throw new ApiError(400, 'At least one active subscription plan is required when subscription is enabled');
      }
    }

    if (normalizedPayload.rentalSubcategoryId) {
      const subcategory = await RentalVehicleSubcategory.findById(normalizedPayload.rentalSubcategoryId).lean();
      if (!subcategory) {
        throw new ApiError(404, 'Rental subcategory not found');
      }
      if (String(normalizedPayload.vehicleCategory || '').toLowerCase() !== 'bike') {
        throw new ApiError(400, 'Rental subcategory can only be assigned to two wheeler vehicles');
      }
      normalizedPayload.rentalSubcategoryName = subcategory.name || '';
    } else {
      normalizedPayload.rentalSubcategoryName = '';
    }
  ...
  export const listRentalVehicleSubcategories = async () => {
    ...
    return {
      ...
      paginator: {
        ...
        links: [
          { url: null, label: "&laquo; Previous", active: false },
          { url: "http://localhost:5000/api/v1/admin/rental-vehicle-subcategories?page=1", label: "1", active: true },
          { url: null, label: "Next &raquo;", active: false }
        ],
        path: "http://localhost:5000/api/v1/admin/rental-vehicle-subcategories"
      }
    };
  };

  export const listPublicRentalVehicleSubcategories = async () => {
    const items = await RentalVehicleSubcategory.find({ active: true, status: 'active' })
      .sort({ createdAt: -1 })
      .lean();
    return items.map(serializeRentalVehicleSubcategory);
  };

  export const createRentalVehicleSubcategory = async (payload) => {
    if (!payload.name?.trim()) {
      throw new ApiError(400, 'Rental subcategory name is required');
    }

    const vehicleCategory = String(payload.vehicleCategory || 'Bike').trim() || 'Bike';
    if (vehicleCategory.toLowerCase() !== 'bike') {
      throw new ApiError(400, 'Rental subcategory can only be created for the two wheeler section');
    }

    const status = payload.status || (normalizeBoolean(payload.active ?? true) ? 'active' : 'inactive');

    const item = await RentalVehicleSubcategory.create({
      name: String(payload.name).trim(),
      short_description: String(payload.short_description || '').trim(),
      description: String(payload.description || '').trim(),
      vehicleCategory,
    ...
    return serializeRentalVehicleSubcategory(item.toObject());
  };

  export const updateRentalVehicleSubcategory = async (id, payload) => {
    const item = await RentalVehicleSubcategory.findById(id);
    if (!item) throw new ApiError(404, 'Rental subcategory not found');
    ...
    if (payload.borderClass !== undefined) {
      item.borderClass = String(payload.borderClass || '').trim();
    }
    if (payload.imageScale !== undefined) {
      item.imageScale = String(payload.imageScale || '').trim();
    }
    if (payload.vehicleCategory !== undefined) {
      const vehicleCategory = String(payload.vehicleCategory || 'Bike').trim() || 'Bike';
      if (vehicleCategory.toLowerCase() !== 'bike') {
        throw new ApiError(400, 'Rental subcategory can only be created for the two wheeler section');
      }
      item.vehicleCategory = vehicleCategory;
    }
    if (payload.status !== undefined) {
      item.status = payload.status || 'active';
      item.active = item.status === 'active';
    ...
    await RentalVehicleType.updateMany(
      { rentalSubcategoryId: item._id },
      { $set: { rentalSubcategoryName: item.name } },
    );

    return serializeRentalVehicleSubcategory(item.toObject());
  };

  export const deleteRentalVehicleSubcategory = async (id) => {
    const deleted = await RentalVehicleSubcategory.findByIdAndDelete(id);
    if (!deleted) throw new ApiError(404, 'Rental subcategory not found');

    await RentalVehicleType.updateMany(
      { rentalSubcategoryId: deleted._id },
      { $set: { rentalSubcategoryId: null, rentalSubcategoryName: '' } },
    );
